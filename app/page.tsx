...
function ProductCard({ item }: { item: string[] }) { return <article className="min-w-0"><div className="aspect-square overflow-hidden rounded-lg bg-[#f1f1f1]"><img src={item[3] || catalogImage} alt={item[0]} className="h-full w-full object-contain p-3" style={{ objectPosition: item[2] }} /></div><h3 className="mt-2 truncate text-[11px] font-bold">{item[0]}</h3><div className="mt-1 flex items-center gap-1 text-[9px] text-amber-500"><Star size={10} fill="currentColor" /> <span className="text-[#777]">4.5/5</span></div><p className="mt-1 text-xs font-bold">{item[1]}</p></article> }
