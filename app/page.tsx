...
export default function HomePage() { return <StorefrontFrame>
  ...
  <section className="rounded-2xl bg-[#f1f1f1] p-6 sm:p-10"><h2 className="text-center text-2xl font-black tracking-[-1px] sm:text-3xl">BROWSE BY CATEGORY</h2><div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{[['T-shirts','t-shirts','/products/category-tshirts.png'],['Shirts','shirts','/products/category-shirts.png'],['Jeans','jeans','/products/category-jeans.png'],['Hoodies','hoodies','/products/category-hoodies.png'],['Shorts','shorts','/products/category-shorts.png'],['Dress Style','dress-style','/products/category-dress-style.png']].map(([name, slug, image]) => <a key={slug} href={`/category/${slug}`} className="relative h-36 overflow-hidden rounded-lg bg-white p-5 text-base font-bold sm:h-44 sm:text-lg"><span className="relative z-10">{name}</span><img src={image} alt={`${name} category`} className="absolute inset-0 h-full w-full object-cover opacity-75 transition-transform duration-300 hover:scale-105" /></a>)}</div></section>
  <ReviewCarousel />
  </StorefrontFrame> }
