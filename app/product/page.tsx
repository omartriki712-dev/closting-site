    ...
    <StorefrontFrame>
      <main className="mx-auto w-full max-w-[1180px] px-3 pb-8 sm:px-0 lg:px-4">
        ...
        <section>
          ...
          {tab === "details" ? <p className="py-5 text-sm leading-relaxed text-muted-foreground">A relaxed graphic tee made from soft cotton with a breathable finish and everyday fit.</p> : tab === "faqs" ? <p className="py-5 text-sm leading-relaxed text-muted-foreground">Questions about shipping, returns, sizing, and care? Our support team is here to help.</p> : <ReviewList onWriteReview={() => reviewDialog.current?.showModal()} />}<dialog ref={reviewDialog} className="fixed inset-0 m-auto h-fit max-h-[90vh] w-[min(92vw,520px)] rounded-2xl border border-border bg-background p-0 shadow-xl backdrop:bg-black/40"><form method="dialog" className="flex flex-col gap-5 p-6"><div className="flex items-center justify-between"><h2 className="text-xl font-bold">Write a Review</h2><button type="submit" formNoValidate aria-label="Close review form" className="text-xl text-muted-foreground">×</button></div><label className="flex flex-col gap-2 text-sm font-medium">Email<input required type="email" className="rounded-lg border border-border px-3 py-2 text-sm font-normal" /></label><label className="flex flex-col gap-2 text-sm font-medium">Full Name<input required type="text" className="rounded-lg border border-border px-3 py-2 text-sm font-normal" /></label><label className="flex flex-col gap-2 text-sm font-medium">Rating<select required defaultValue="5" className="rounded-lg border border-border px-3 py-2 text-sm font-normal"><option value="5">5 stars</option><option value="4">4 stars</option><option value="3">3 stars</option><option value="2">2 stars</option><option value="1">1 star</option></select></label><label className="flex flex-col gap-2 text-sm font-medium">Comment<textarea required rows={4} className="resize-none rounded-lg border border-border px-3 py-2 text-sm font-normal" /></label><button value="submit" className="rounded-full bg-black px-5 py-3 text-sm font-medium text-white">Submit Review</button></form></dialog>
        </section>
