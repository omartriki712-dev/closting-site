    ...
    <StorefrontFrame>
      <main className="mx-auto w-full max-w-[1180px] px-3 pb-8 sm:px-0 lg:px-4">
        ...
        <section className="border-b border-border py-4 lg:col-start-2 lg:row-start-2">
          ...
          <div className="flex gap-2">{[["olive", "#514d35"], ["green", "#31504c"], ["navy", "#2f334e"]].map(([name, value]) => <button key={name} aria-label={`Select ${name} color`} onClick={() => setColor(name)} className={`grid size-8 place-items-center rounded-full ${color === name ? "ring-2 ring-offset-2 ring-foreground" : ""}`} style={{ backgroundColor: value }}>{color === name && <span className="text-xs text-white">✓</span>}</button>)}</div>
        </section>

        <section className="border-b border-border py-4 lg:col-start-2 lg:row-start-3"><p className="mb-3 text-[12px] text-muted-foreground">Choose Size</p><div className="flex gap-2">{["Small", "Medium", "Large", "X-Large"].map((item) => <button key={item} onClick={() => setSize(item)} className={`flex-1 rounded-full px-2 py-2 text-[10px] ${size === item ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>{item}</button>)}</div></section>

        <section className="flex gap-2 py-4 lg:col-start-2 lg:row-start-4"><div className="flex items-center justify-between rounded-full bg-muted px-4 text-sm"><button aria-label="Decrease quantity" onClick={() => setQuantity(Math.max(1, quantity - 1))}><Minus size={13} /></button><span className="px-4">{quantity}</span><button aria-label="Increase quantity" onClick={() => setQuantity(quantity + 1)}><Plus size={13} /></button></div><button className="flex-1 rounded-full bg-black px-6 py-3.5 text-sm font-medium text-white">Add to Cart</button></section></div>

        <section>
          <div className="flex border-b border-border text-sm"><button onClick={() => setTab("details")} className={`flex-1 border-b-2 py-3 ${tab === "details" ? "border-black font-semibold" : "border-transparent text-muted-foreground"}`}>Product Details</button><button onClick={() => setTab("reviews")} className={`flex-1 border-b-2 py-3 ${tab === "reviews" ? "border-black font-semibold" : "border-transparent text-muted-foreground"}`}>Rating & Reviews</button><button onClick={() => setTab("faqs")} className={`flex-1 border-b-2 py-3 ${tab === "faqs" ? "border-black font-semibold" : "border-transparent text-muted-foreground"}`}>FAQs</button></div>
          {tab === "details" ? <p className="py-5 text-sm leading-relaxed text-muted-foreground">A relaxed graphic tee made from soft cotton with a breathable finish and everyday fit.</p> : tab === "faqs" ? <p className="py-5 text-sm leading-relaxed text-muted-foreground">Questions about shipping, returns, sizing, and care? Our support team is here to help.</p> : <ReviewList onWriteReview={() => reviewDialog.current?.showModal()} />}<dialog ref={reviewDialog} className="fixed inset-0 m-auto h-fit max-h-[90vh] w-[min(92vw,520px)] rounded-2xl border border-border bg-background p-0 shadow-xl backdrop:bg-black/40"><form method="dialog" className="flex flex-col gap-5 p-6"><div className="flex items-center justify-between"><h2 className="text-xl font-bold">Write a Review</h2><button type="submit" formNoValidate aria-label="Close review form" className="text-xl text-muted-foreground">×</button></div><label className="flex flex-col gap-2 text-sm font-medium">Email<input required type="email" className="rounded-lg border border-border bg-background px-3 py-2 text-sm font-normal text-foreground" /></label><label className="flex flex-col gap-2 text-sm font-medium">Full Name<input required type="text" className="rounded-lg border border-border bg-background px-3 py-2 text-sm font-normal text-foreground" /></label><label className="flex flex-col gap-2 text-sm font-medium">Rating<select required defaultValue="5" className="rounded-lg border border-border bg-background px-3 py-2 text-sm font-normal text-foreground [color-scheme:light] dark:[color-scheme:dark]"><option value="5">5 stars</option><option value="4">4 stars</option><option value="3">3 stars</option><option value="2">2 stars</option><option value="1">1 star</option></select></label><label className="flex flex-col gap-2 text-sm font-medium">Comment<textarea required rows={4} className="resize-none rounded-lg border border-border px-3 py-2 text-sm font-normal" /></label><button value="submit" className="rounded-full bg-black px-5 py-3 text-sm font-medium text-white">Submit Review</button></form></dialog>
        </section>
