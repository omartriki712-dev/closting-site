...
const styles = [['Casual', '/category/dress-style'], ['Formal', '/category/dress-style'], ['Party', '/category/dress-style'], ['Gym', '/category/dress-style']] as const
...
export function StorefrontHeader() {
  ...
  return <>
    ...
    <header className="relative mx-4 flex h-20 items-center justify-between border-b border-[#eee] sm:mx-8 lg:mx-12">
      ...
      <nav className={`${menuOpen ? 'absolute left-0 top-20 z-30 flex' : 'hidden'} flex-col gap-4 bg-background p-5 text-lg shadow-md sm:static sm:flex sm:flex-row sm:items-center sm:justify-center sm:gap-8 sm:bg-transparent sm:p-0 sm:text-lg sm:shadow-none lg:absolute lg:left-1/2 lg:w-auto lg:-translate-x-1/2`}><NavigationMenu className="relative"><NavigationMenuList><NavigationMenuItem><NavigationMenuTrigger onClick={() => router.push('/shop')} className="text-lg font-medium text-foreground hover:bg-transparent focus:bg-transparent">Shop</NavigationMenuTrigger><NavigationMenuContent className="left-1/2 w-[min(92vw,560px)] -translate-x-1/2 p-6 text-[#111]"><div className="flex w-full gap-3 sm:gap-4"><a href="/category/new-arrivals" className="group min-w-0 flex-1 overflow-hidden rounded-xl bg-[#f4f4f4]"><img src="/products/category-dress-style.png" alt="New arrivals" className="h-40 w-full object-cover transition-transform duration-300 group-hover:scale-105" /><span className="block p-3 text-sm font-semibold">New arrivals</span></a><a href="/category/sale" className="group min-w-0 flex-1 overflow-hidden rounded-xl bg-[#f4f4f4]"><img src="/products/category-hoodies.png" alt="Sale styles" className="h-40 w-full object-cover transition-transform duration-300 group-hover:scale-105" /><span className="block p-3 text-sm font-semibold">Sale styles</span></a></div></NavigationMenuContent></NavigationMenuItem></NavigationMenuList></NavigationMenu><a href="/category/sale">On Sale</a><a href="/category/new-arrivals">New Arrivals</a><a href="/brands">Brands</a></nav>
      <div className="flex items-center justify-end gap-4 lg:w-1/3"><button type="button" onClick={() => setSearchOpen(true)} aria-label="Open search"><Search size={20} /></button><ThemeToggle /><a href="/cart" aria-label="Shopping cart"><ShoppingCart size={20} /></a><a href="/login" aria-label="Log in"><CircleUserRound size={20} /></a></div>
    </header>
    {searchOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 px-4" role="presentation" onMouseDown={(event) => event.currentTarget === event.target && setSearchOpen(false)}><div role="dialog" aria-modal="true" aria-label="Search products" className="w-full max-w-xl rounded-2xl bg-background p-5 shadow-2xl"><div className="flex items-center justify-between"><h2 className="text-lg font-bold">Search products</h2><button onClick={() => setSearchOpen(false)} aria-label="Close search"><X size={20} /></button></div><div className="mt-5 flex items-center gap-3 rounded-full border border-[#ddd] px-4 py-3"><Search size={18} /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="What are you looking for?" className="w-full bg-transparent text-sm outline-none" /></div><p className="mt-4 text-xs text-[#777]">{query ? `Showing results for “${query}”` : 'Try searching for shirts, jeans, or new arrivals.'}</p></div></div>}
  </>
}

export function StorefrontFrame({ children }: { children: React.ReactNode }) { return <div className="min-h-screen w-full overflow-x-hidden bg-background text-foreground"><StorefrontHeader /><main className="page-enter mx-auto w-[calc(100%-40px)] max-w-[1280px]">{children}</main><StorefrontNewsletter /><StorefrontFooter /></div> }
